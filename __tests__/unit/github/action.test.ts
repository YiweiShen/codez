...
describe('createProgressComment', () => {
  ...
  it('posts a PR review reply comment with unchecked steps and returns comment id', async () => {
    ...
    const octokit = {
      rest: { pulls: { createReplyForReviewComment: mockReply } },
    } as unknown as Octokit;
    const event = {
      pull_request: { number: 2 },
      comment: { id: 10, in_reply_to_id: undefined },
...
describe('updateProgressComment', () => {
  ...
  it('updates an issue comment with provided steps', async () => {
    ...
    const octokit = {
      rest: { issues: { updateComment: mockUpdate } },
    } as unknown as Octokit;
  ...
  it('updates a PR review comment with provided steps', async () => {
    ...
    const octokit = {
      rest: { pulls: { updateReviewComment: mockReview } },
    } as unknown as Octokit;
...
describe('createIssuesFromFeaturePlan', () => {
  ...
  it('creates issues for valid JSON feature plan', async () => {
    ...
    const postSpy = jest
      .spyOn(githubHelpers, 'postComment')
      .mockResolvedValue();
    ...
    expect(postSpy).toHaveBeenCalledWith(
      ...
      repo,
      event,
      'Created new feature issue #11 for "T1"',
    );
    expect(postSpy).toHaveBeenCalledWith(
      octokit,
      repo,
      event,
      'Created new feature issue #22 for "T2"',
    );
  });
