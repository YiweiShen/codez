...
describe('loadEventPayload', () => {
  it('parses valid JSON', () => {
    ...
    const result = loadEventPayload(filePath);
  ...
  it('throws error for invalid JSON', () => {
    ...
    expect(() => loadEventPayload(filePath)).toThrow(/Failed to read or parse event payload/);
  });
});

describe('processEvent', () => {
  it('returns direct prompt bypass', () => {
    const config: any = { directPrompt: 'my-prompt' };
    const result = processEvent(config);
  ...
  it('skips issuesAssigned when assignee not in trigger list', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ev-'));
  ...
  it('triggers issuesAssigned when assignee in trigger list', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ev-'));
  ...
  it('returns null for comment event without trigger phrase', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ev-'));
  ...
  it('processes comment event with flags and prompt', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ev-'));
